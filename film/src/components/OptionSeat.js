        ...
        <div className="row">
          <div className="screen">
            <img
              ...
              style={{ width: "45rem", height: "3rem" }}
            />
            <span className="screen-text">Màn hình</span>
          </div>

          {this.props.seat.danhSachGhe.map((item, index) => {
            if (item.daDat) {
              return (
              ...
              return (
                <div className="card" style={{ width: "5rem" }} key={index}>
                  <div className="list-group list-group-flush">
                    <button className="list-group-item seat-active">
                      {item.tenGhe}
                    </button>
                  </div>
                </div>
        ...
        <div className="card" style={{ width: "18rem" }}>
          ...
          <div className="card-body">
            ...
            <h6 className="card-text">
              {seat.thongTinPhim.tenCumRap} | {seat.thongTinPhim.tenRap}
            </h6>
            <p>Địa chỉ: {seat.thongTinPhim.diaChi}</p>
            <h6>
              Ngày chiếu:{" "}
              {new Date(seat.thongTinPhim.ngayChieu).toLocaleDateString(
                "en-GB"
              )}
            </h6>
  ...
  render() {
    return (
      <div className="container">
        <div className="row">
          <div className="col-sm-8">{this.renderTable()}</div>
          <div className="col-sm-4">{this.renderItem()}</div>
        </div>
      </div>
    );
  }
